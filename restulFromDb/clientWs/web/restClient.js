...
// Model for Chef entity
    models.Chef = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/webServiceFromDb/webresources/entities.chef/",
        idAttribute: 'nochef',
        defaults: {
            nomch: "",
            prenomch: ""
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('nochef');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/webServiceFromDb/webresources/entities.chef/';
            }
    ...
    // Collection class for Chef entities
    models.ChefCollection = Backbone.Collection.extend({
        model: models.Chef,
        url: "http://localhost:8080/webServiceFromDb/webresources/entities.chef/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ChefCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-chef-list-itemis template identifier for item
                        templateName: '#tpl-chef-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.chef = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.chef,
                    // tpl-chef-details is template identifier for chosen model element
                    templateName: '#tpl-chef-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Chef();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-chef-details is a template identifier for chosen model element
                templateName: '#tpl-chef-details',
